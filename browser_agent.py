...
from autogen_magentic_one.messages import RequestReplyMessage, UserContent
...
async def main(search_query: str = None) -> None:
    # Create the runtime.
    ...
    # If a search query is provided, send it as a message
    if search_query:
        await runtime.send_message(RequestReplyMessage(content=UserContent(search_query)), user_proxy.id)
    else:
